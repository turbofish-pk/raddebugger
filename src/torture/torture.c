...
#if OS_WINDOWS
    g_radbin_path = os_full_path_from_path(arena, str8_lit("radbin.exe"));
#else
    g_radbin_path = os_full_path_from_path(arena, str8_lit("radbin"));
#endif
  }
...
#if OS_WINDOWS
    Temp scratch = scratch_begin(0, 0);
    path = os_full_path_from_path(arena, str8f(scratch.arena, "%S.exe", raddbg_base_name));
    scratch_end(scratch);
#else
    path = os_full_path_from_path(arena, raddbg_base_name);
#endif
    AssertAlways(path.size);
  }
  return path;
...
t_cwd_path(void)
{
  ...
  if (path[0] == 0) {
    ...
    // should os_get_current_path return directory paths with slash or no slash
...
t_invoke_env(String8 exe_path, String8 cmdline, String8List env, U64 timeout_us)
{
  ...
  OS_Handle read_capture_handles[2] = {0}, write_capture_handles[2] = {0};
  ...
  B32  is_process_live          = 1;
  B32  is_stdout_live           = 1;
  B32  is_stderr_live           = 1;
