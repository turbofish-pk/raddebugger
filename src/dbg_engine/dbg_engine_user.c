...
d_trap_net_from_thread__step_over_line(Arena *arena, D_Entity *thread)
{
  ...
  if(good_line_info)
  {
    ...
    D_ProcessMemorySlice machine_code_slice = d_process_memory_slice_from_vaddr_range(scratch.arena, process->handle, line_vaddr_rng, 0, os_now_microseconds()+10000000);
    machine_code = machine_code_slice.data;
